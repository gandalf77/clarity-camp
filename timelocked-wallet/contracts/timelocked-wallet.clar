...
(define-public (lock (new-beneficiary principal) (unlock-at uint) (amount uint))
	(begin
		...
		(try! (stx-transfer? amount tx-sender (as-contract tx-sender))) ;; why do we use as-contract here?
		;; #[filter(new-beneficiary)]
    (var-set beneficiary (some new-beneficiary)) ;; why do we make new-beneficiary an optional here?
