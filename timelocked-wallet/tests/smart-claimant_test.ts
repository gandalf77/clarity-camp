...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        const amount = 1000; // be sure to pick a test amount that is divisible by 4
        const share = Math.floor(amount / 4);
        chain.mineBlock([
            Tx.contractCall('timelocked-wallet', 'lock', [types.principal(beneficiary),
            types.uint(unlock_height), types.uint(amount)], deployer.address)
        ]);
